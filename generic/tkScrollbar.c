...
static Tk_ConfigSpec configSpecs[] =
{
	...
	NULL, Tk_Offset(TkScrollbar, imagePtr), 0},
	...
	&bltTileOption},
    {TK_CONFIG_CUSTOM, "-tile2", "tile2", "Tile",
    ...
    ClientData clientData;	/* Information about window. */
{
	...
	border = scrollPtr->bgBorder;
	relief = TK_RELIEF_RAISED;
        tile = (scrollPtr->tile2 ? scrollPtr->tile2 : scrollPtr->tile);
    }
    if (scrollPtr->vertical) {
	if (Blt_HasTile(tile)) {
    ...
    if (scrollPtr->image != NULL) {
        ...
        int imgW, imgH, xLen, yLen, yStart, xStart, yOfs = 0, xOfs = 0, pad = 1;
